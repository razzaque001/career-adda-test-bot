...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if not os.path.exists(test_path):
            await context.bot.send_message(chat_id=chat_id, text="❌ Test not found.")
            return
        ...
        try:
            ...
            await context.bot.send_message(chat_id=chat_id, text="❌ Corrupted test file.")
            return

        if not questions:
            await context.bot.send_message(chat_id=chat_id, text="❌ Empty test.")
            return
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"🚀 *{test_name.upper()} TEST STARTED*\n\n"
...
async def send_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Skip button
    keyboard.append([InlineKeyboardButton("⏭ Skip", callback_data=f"skip_{idx}")])

    # Submit button
    keyboard.append([InlineKeyboardButton("✅ Submit Test", callback_data="submit_test")])
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("skip_"):
        ...
        context.user_data['skipped_questions'].append(skipped_question)
        ...
        if context.user_data['current_idx'] < len(context.user_data['quiz']):
            await send_question(update, context)
        else:
            # Reattempt skipped questions
            if context.user_data['skipped_questions']:
                context.user_data['quiz'] = context.user_data['skipped_questions']
                context.user_data['skipped_questions'] = []
                context.user_data['current_idx'] = 0
    ...
    selected_option = context.user_data['current_options'][opt_idx]
...
async def finish_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    attempted = score + len(context.user_data['skipped_questions'])
    ...
    # Show tests again automatically
    await context.bot.send_message(
        chat_id=chat_id,
